...
public class ServerStatisticsManager extends StatisticsManager
{
        ...
        try (JsonReader jsonreader = new JsonReader(new StringReader(p_199062_2_)))
        {
            ...
            if (compoundnbt.contains("stats", 10))
            {
                ...
                for (String s : compoundnbt1.keySet())
                {
                    ...
                    {
                        Util.acceptOrElse(Registry.STATS.func_241873_b(new ResourceLocation(s)), (p_219731_3_) ->
                        {
    ...
    {
        return Optional.ofNullable(ResourceLocation.tryCreate(p_219728_2_)).flatMap(p_219728_1_.getRegistry()::func_241873_b).map(p_219728_1_::get);
    }
