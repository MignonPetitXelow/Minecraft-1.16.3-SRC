...
public class NBTDynamicOps implements DynamicOps<INBT>
{
    ...
    public DataResult<INBT> mergeToList(INBT p_mergeToList_1_, INBT p_mergeToList_2_)
    {
        ...
        {
            CollectionNBT<?> collectionnbt = func_240602_a_(p_mergeToList_1_ instanceof CollectionNBT ? ((CollectionNBT)p_mergeToList_1_).func_230528_d__() : 0, p_mergeToList_2_.getId());
    ...
    public DataResult<INBT> mergeToList(INBT p_mergeToList_1_, List<INBT> p_mergeToList_2_)
    {
        ...
        {
            CollectionNBT<?> collectionnbt = func_240602_a_(p_mergeToList_1_ instanceof CollectionNBT ? ((CollectionNBT)p_mergeToList_1_).func_230528_d__() : 0, p_mergeToList_2_.stream().findFirst().map(INBT::getId).orElse((byte)0));
    ...
    class NBTRecordBuilder extends AbstractStringBuilder<INBT, CompoundNBT>
    {
        ...
        protected DataResult<INBT> build(CompoundNBT p_build_1_, INBT p_build_2_)
        {
            if (p_build_2_ != null && p_build_2_ != EndNBT.INSTANCE)
            {
                ...
                {
                    CompoundNBT compoundnbt = new CompoundNBT(Maps.newHashMap(((CompoundNBT)p_build_2_).func_240600_h_()));

                    for (Entry<String, INBT> entry : p_build_1_.func_240600_h_().entrySet())
                    {
