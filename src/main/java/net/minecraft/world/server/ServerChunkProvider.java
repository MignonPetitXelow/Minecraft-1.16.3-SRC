...
public class ServerChunkProvider extends AbstractChunkProvider
{
    ...
    public IChunk getChunk(int chunkX, int chunkZ, ChunkStatus requiredStatus, boolean load)
    {
        ...
        else
        {
            ...
            IChunk ichunk1 = completablefuture.join().map((p_lambda$getChunk$1_0_) ->
            {
                return p_lambda$getChunk$1_0_;
            }, (p_lambda$getChunk$2_1_) ->
            {
                if (load)
                {
                    throw(IllegalStateException)Util.pauseDevMode(new IllegalStateException("Chunk not there when requested: " + p_lambda$getChunk$2_1_));
                }
                else {
                    return null;
                }
    ...
    public Chunk getChunkNow(int chunkX, int chunkZ)
    {
        ...
        else
        {
            ...
            {
                Either<IChunk, ChunkHolder.IChunkLoadingError> either = chunkholder.func_225410_b(ChunkStatus.FULL).getNow((Either)null);
    ...
    public CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_217232_b(int chunkX, int chunkZ, ChunkStatus requiredStatus, boolean load)
    {
        ...
        else
        {
            ...
            }, this.executor).thenCompose((p_lambda$getFutureChunk$4_0_) ->
            {
                return p_lambda$getFutureChunk$4_0_;
            });
        }
    ...
    private CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_217233_c(int chunkX, int chunkZ, ChunkStatus requiredStatus, boolean load)
    {
        ...
        int j = 65 + ChunkStatus.getDistance(requiredStatus);
        ChunkHolder chunkholder = this.func_217213_a(i);
    ...
    public boolean chunkExists(int x, int z)
    {
        ...
        int i = 65 + ChunkStatus.getDistance(ChunkStatus.FULL);
    ...
    private void tickChunks()
    {
        ...
        if (!flag)
        {
            ...
            list.forEach((p_lambda$tickChunks$5_7_) ->
            {
                Optional<Chunk> optional = p_lambda$tickChunks$5_7_.getTickingFuture().getNow(ChunkHolder.UNLOADED_CHUNK).left();

                if (optional.isPresent())
                {
                    this.world.getProfiler().startSection("broadcast");
                    p_lambda$tickChunks$5_7_.sendChanges(optional.get());
                    this.world.getProfiler().endSection();
                    Optional<Chunk> optional1 = p_lambda$tickChunks$5_7_.getEntityTickingFuture().getNow(ChunkHolder.UNLOADED_CHUNK).left();

                    if (optional1.isPresent())
                    {
                        Chunk chunk = optional1.get();
                        ChunkPos chunkpos = p_lambda$tickChunks$5_7_.getPosition();
    ...
    final class ChunkExecutor extends ThreadTaskExecutor<Runnable>
    {
        ...
        {
            super("Chunk source main thread executor for " + worldIn.getDimensionKey().func_240901_a_());
        }
