...
public abstract class TicketManager
{
    ...
    private static final int PLAYER_TICKET_LEVEL = 65 + ChunkStatus.getDistance(ChunkStatus.FULL) - 2;
    ...
    {
        this.register(pos.asLong(), new Ticket<>(type, 65 - distance, value));
    }

    public <T> void release(TicketType<T> type, ChunkPos pos, int distance, T value)
    {
        Ticket<T> ticket = new Ticket<>(type, 65 - distance, value);
        this.release(pos.asLong(), ticket);
    }

    private SortedArraySet < Ticket<? >> getTicketSet(long p_229848_1_)
    {
        return this.tickets.computeIfAbsent(p_229848_1_, (p_lambda$getTicketSet$6_0_) ->
        {
    ...
    public void updatePlayerPosition(SectionPos sectionPosIn, ServerPlayerEntity player)
    {
        ...
        this.playersByChunkPos.computeIfAbsent(i, (p_lambda$updatePlayerPosition$7_0_) ->
        {
