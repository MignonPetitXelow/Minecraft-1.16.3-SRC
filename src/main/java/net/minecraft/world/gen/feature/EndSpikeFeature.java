...
public class EndSpikeFeature extends Feature<EndSpikeFeatureConfig>
{
    ...
    private void placeSpike(IServerWorld worldIn, Random rand, EndSpikeFeatureConfig config, EndSpikeFeature.EndSpike spike)
    {
        ...
        for (BlockPos blockpos : BlockPos.getAllInBoxMutable(new BlockPos(spike.getCenterX() - i, 0, spike.getCenterZ() - i), new BlockPos(spike.getCenterX() + i, spike.getHeight() + 10, spike.getCenterZ() + i)))
        {
            ...
            {
                this.func_230367_a_(worldIn, blockpos, Blocks.OBSIDIAN.getDefaultState());
            }
            else if (blockpos.getY() > 65)
            {
                this.func_230367_a_(worldIn, blockpos, Blocks.AIR.getDefaultState());
            }
        }
                ...
                for (int l = -2; l <= 2; ++l)
                {
                    for (int i1 = 0; i1 <= 3; ++i1)
                    {
                        ...
                        if (flag || flag1 || flag2)
                        {
                            ...
                            this.func_230367_a_(worldIn, blockpos$mutable.setPos(spike.getCenterX() + k, spike.getHeight() + i1, spike.getCenterZ() + l), blockstate);
                        }
                    }
                }
            }
        ...
        this.func_230367_a_(worldIn, new BlockPos(spike.getCenterX(), spike.getHeight(), spike.getCenterZ()), Blocks.BEDROCK.getDefaultState());
    }
