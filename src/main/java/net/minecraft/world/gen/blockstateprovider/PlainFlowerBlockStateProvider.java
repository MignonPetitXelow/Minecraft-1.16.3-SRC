...
public class PlainFlowerBlockStateProvider extends BlockStateProvider
{
    ...
    public BlockState getBlockState(Random randomIn, BlockPos blockPosIn)
    {
        ...
        if (d0 < -0.8D)
        {
            return Util.getRandomObject(field_227402_b_, randomIn);
        }
        else
        {
            return randomIn.nextInt(3) > 0 ? Util.getRandomObject(field_227403_c_, randomIn) : Blocks.DANDELION.getDefaultState();
        }
    }
