...
public class AnvilSaveConverter
{
    ...
    static boolean convertRegions(SaveFormat.LevelSave levelSave, IProgressUpdate progress)
    {
        ...
        IServerConfiguration iserverconfiguration = levelSave.readServerConfiguration(worldsettingsimport, DatapackCodec.field_234880_a_);
        long j = iserverconfiguration != null ? iserverconfiguration.getDimensionGeneratorSettings().getSeed() : 0L;
        Registry<Biome> registry = dynamicregistries$impl.func_243612_b(Registry.BIOME_KEY);
        ...
        func_242983_a(dynamicregistries$impl, new File(file2, "region"), list1, new SingleBiomeProvider(registry.func_243576_d(Biomes.NETHER_WASTES)), list.size(), i, progress);
        func_242983_a(dynamicregistries$impl, new File(file3, "region"), list2, new SingleBiomeProvider(registry.func_243576_d(Biomes.THE_END)), list.size() + list1.size(), i, progress);
