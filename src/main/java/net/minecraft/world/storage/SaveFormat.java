...
public class SaveFormat
{
    ...
    private static <T> Pair<DimensionGeneratorSettings, Lifecycle> getSettingLifecyclePair(Dynamic<T> nbt, DataFixer fixer, int version)
    {
        ...
        Dynamic<T> dynamic1 = fixer.update(TypeReferences.field_233375_y_, dynamic, version, SharedConstants.getVersion().getWorldVersion());
        ...
        {
            Registry<DimensionType> registry = RegistryLookupCodec.func_244331_a(Registry.DIMENSION_TYPE_KEY).codec().parse(dynamic1).resultOrPartial(Util.func_240982_a_("Dimension type registry: ", LOGGER::error)).orElseThrow(() -> {
                return new IllegalStateException("Failed to get dimension registry");
            });
            Registry<Biome> registry1 = RegistryLookupCodec.func_244331_a(Registry.BIOME_KEY).codec().parse(dynamic1).resultOrPartial(Util.func_240982_a_("Biome registry: ", LOGGER::error)).orElseThrow(() -> {
                return new IllegalStateException("Failed to get biome registry");
            });
            Registry<DimensionSettings> registry2 = RegistryLookupCodec.func_244331_a(Registry.field_243549_ar).codec().parse(dynamic1).resultOrPartial(Util.func_240982_a_("Noise settings registry: ", LOGGER::error)).orElseThrow(() -> {
    ...
    {
        return DatapackCodec.field_234881_b_.parse(nbt).resultOrPartial(LOGGER::error).orElse(DatapackCodec.field_234880_a_);
    }
    ...
    private static DatapackCodec readWorldDatapackCodec(File levelDat, DataFixer fixer)
    {
        try
        {
            CompoundNBT compoundnbt = CompressedStreamTools.func_244263_a(levelDat);
            ...
            Dynamic<INBT> dynamic = fixer.update(DefaultTypeReferences.LEVEL.func_219816_a(), new Dynamic<>(NBTDynamicOps.INSTANCE, compoundnbt1), i, SharedConstants.getVersion().getWorldVersion());
            return dynamic.get("DataPacks").result().map(SaveFormat::decodeDatapackCodec).orElse(DatapackCodec.field_234880_a_);
        }
        catch (Exception exception)
        {
    ...
    private static BiFunction<File, DataFixer, ServerWorldInfo> readServerWorldInfo(DynamicOps<INBT> nbt, DatapackCodec datapackCodec)
    {
        return (p_242976_2_, p_242976_3_) ->
        {
            try {
                ...
                compoundnbt1.remove("Player");
                int i = compoundnbt1.contains("DataVersion", 99) ? compoundnbt1.getInt("DataVersion") : -1;
                Dynamic<INBT> dynamic = p_242976_3_.update(DefaultTypeReferences.LEVEL.func_219816_a(), new Dynamic<>(nbt, compoundnbt1), i, SharedConstants.getVersion().getWorldVersion());
        ...
        return (p_242977_3_, p_242977_4_) ->
        {
            try {
                ...
                if (j != 19132 && j != 19133)
                {
                    ...
                    DatapackCodec datapackcodec = dynamic.get("DataPacks").result().map(SaveFormat::decodeDatapackCodec).orElse(DatapackCodec.field_234880_a_);
                    WorldSettings worldsettings = WorldSettings.decodeWorldSettings(dynamic, datapackcodec);
    ...
    public class LevelSave implements AutoCloseable
    {
        ...
        {
            return DimensionType.func_236031_a_(dimensionKey, this.saveDir.toFile());
        }
