...
public class EnchantmentHelper
{
    ...
    public static Map<Enchantment, Integer> deserializeEnchantments(ListNBT serialized)
    {
        ...
        for (int i = 0; i < serialized.size(); ++i)
        {
            ...
            Registry.ENCHANTMENT.func_241873_b(ResourceLocation.tryCreate(compoundnbt.getString("id"))).ifPresent((enchantment) ->
            {
    ...
    private static void applyEnchantmentModifier(EnchantmentHelper.IEnchantmentVisitor modifier, ItemStack stack)
    {
        if (!stack.isEmpty())
        {
            ...
            for (int i = 0; i < listnbt.size(); ++i)
            {
                ...
                Registry.ENCHANTMENT.func_241873_b(ResourceLocation.tryCreate(s)).ifPresent((enchantment) ->
                {
    ...
    {
        return func_234844_a_(targetEnchantment, entityIn, (stack) ->
        {
            return true;
        });
    }
    ...
    @Nullable
    public static Entry<EquipmentSlotType, ItemStack> func_234844_a_(Enchantment enchantment, LivingEntity livingEntity, Predicate<ItemStack> stackCondition)
    {
