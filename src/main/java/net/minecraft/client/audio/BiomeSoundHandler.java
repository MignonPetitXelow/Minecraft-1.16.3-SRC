...
public class BiomeSoundHandler implements IAmbientSoundHandler
{
    ...
    public void tick()
    {
        ...
        Biome biome = this.biomeManager.func_235198_a_(this.player.getPosX(), this.player.getPosY(), this.player.getPosZ());

        if (biome != this.currentBiome)
        {
            this.currentBiome = biome;
            this.currentAmbientMoodSound = biome.func_235092_v_();
            this.currentAmbientAdditionalSound = biome.func_235093_w_();
        ...
        {
            if (this.random.nextDouble() < currentAmbientAdditionalSound.func_235024_b_())
            {
        ...
        this.currentAmbientMoodSound.ifPresent((currentAmbientMoodSound) ->
        {
            ...
            int i = currentAmbientMoodSound.func_235037_c_() * 2 + 1;
            BlockPos blockpos = new BlockPos(this.player.getPosX() + (double)this.random.nextInt(i) - (double)currentAmbientMoodSound.func_235037_c_(), this.player.getPosYEye() + (double)this.random.nextInt(i) - (double)currentAmbientMoodSound.func_235037_c_(), this.player.getPosZ() + (double)this.random.nextInt(i) - (double)currentAmbientMoodSound.func_235037_c_());
            ...
            if (j > 0)
            {
                ...
                this.darknessAmbienceChance -= (float)(world.getLightFor(LightType.BLOCK, blockpos) - 1) / (float)currentAmbientMoodSound.func_235035_b_();
            }
