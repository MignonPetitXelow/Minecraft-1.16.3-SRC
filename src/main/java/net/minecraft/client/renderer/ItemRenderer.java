...
public class ItemRenderer implements IResourceManagerReloadListener
{
    ...
    {
        boolean flag = Config.getTextureMap().isTextureBound();
        boolean flag1 = Config.isMultiTexture() && flag;

        if (flag1)
        {
            bufferIn.setBlockLayer(RenderTypes.SOLID);
        }
        ...
        if (flag1)
        {
            bufferIn.setBlockLayer((RenderType)null);
                ...
                else
                {
                    ...
                    if (EmissiveTextures.isActive())
                    {
                        if (EmissiveTextures.hasEmissive())
                        {
                            ...
                            ivertexbuilder = getBuffer(bufferIn, rendertype, true, false);
                            this.renderModel(modelIn, itemStackIn, LightTexture.MAX_BRIGHTNESS, combinedOverlayIn, matrixStackIn, ivertexbuilder);
    ...
    public static IVertexBuilder getArmorVertexBuilder(IRenderTypeBuffer buffer, RenderType renderType, boolean noEntity, boolean withGlint)
    {
        ...
        return withGlint ? VertexBuilderUtils.newDelegate(buffer.getBuffer(noEntity ? RenderType.func_239270_k_() : RenderType.func_239271_l_()), buffer.getBuffer(renderType)) : buffer.getBuffer(renderType);
    }
    ...
    {
        return VertexBuilderUtils.newDelegate(new MatrixApplyingVertexBuilder(buffer.getBuffer(RenderType.func_239273_n_()), matrixEntry.getMatrix(), matrixEntry.getNormal()), buffer.getBuffer(renderType));
    }
    ...
    public static IVertexBuilder getEntityGlintVertexBuilder(IRenderTypeBuffer buffer, RenderType renderType, boolean noEntity, boolean withGlint)
    {
        ...
        return withGlint ? VertexBuilderUtils.newDelegate(buffer.getBuffer(noEntity ? RenderType.func_239273_n_() : RenderType.func_239274_p_()), buffer.getBuffer(renderType)) : buffer.getBuffer(renderType);
    }
