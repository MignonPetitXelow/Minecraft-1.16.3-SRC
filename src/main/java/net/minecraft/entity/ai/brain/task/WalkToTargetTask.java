...
public class WalkToTargetTask extends Task<MobEntity>
{
    ...
    {
        if (entityIn.getBrain().hasMemory(MemoryModuleType.WALK_TARGET) && !this.hasReachedTarget(entityIn, entityIn.getBrain().getMemory(MemoryModuleType.WALK_TARGET).get()) && entityIn.getNavigator().t())
        {
