...
public abstract class MobEntity extends LivingEntity
{
    ...
    {
        if (Reflector.IForgeItem_getEquipmentSlot.exists())
        {
            EquipmentSlotType equipmentslottype = (EquipmentSlotType)Reflector.call(stack.getItem(), Reflector.IForgeItem_getEquipmentSlot, stack);
        ...
        if (item != Blocks.CARVED_PUMPKIN.asItem() && (!(item instanceof BlockItem) || !(((BlockItem)item).getBlock() instanceof AbstractSkullBlock)))
        {
            ...
            else
            {
                ...
                {
                    flag = Reflector.callBoolean(stack.getItem(), Reflector.IForgeItem_isShield, stack, null);
                }

                return flag ? EquipmentSlotType.OFFHAND : EquipmentSlotType.MAINHAND;
            }
        }
        else
        {
        ...
        else
        {
            ...
            if (itemstack.getItem() instanceof SpawnEggItem)
            {
                if (this.world instanceof ServerWorld)
                {
                    ...
                    Optional<MobEntity> optional = spawneggitem.func_234809_a_(p_233661_1_, this, (EntityType<? extends MobEntity>) this.getType(), (ServerWorld) this.world, this.getPositionVec(), itemstack);
    ...
    {
        boolean flag = ReflectorForge.canDisableShield(p_233655_2_, p_233655_3_, p_233655_1_, this);
        boolean flag1 = ReflectorForge.isShield(p_233655_3_, p_233655_1_);

        if (!p_233655_2_.isEmpty() && !p_233655_3_.isEmpty() && flag && flag1)
        {
            float f = 0.25F + (float)EnchantmentHelper.getEfficiencyModifier(this) * 0.05F;

            if (this.rand.nextFloat() < f)
            {
                p_233655_1_.getCooldownTracker().setCooldown(p_233655_3_.getItem(), 100);
