...
public abstract class PlayerEntity extends LivingEntity
{
    ...
    public Vector3d func_241843_o(float p_241843_1_)
    {
        ...
        float f = MathHelper.lerp(p_241843_1_ * 0.5F, this.rotationPitch, this.prevRotationPitch) * ((float)Math.PI / 180F);
        float f1 = MathHelper.lerp(p_241843_1_, this.prevRenderYawOffset, this.renderYawOffset) * ((float)Math.PI / 180F);

        if (!this.isElytraFlying() && !this.isSpinAttacking())
        {
            if (this.isActualySwimming())
            {
                return this.func_242282_l(p_241843_1_).add((new Vector3d(d0, 0.2D, -0.15D)).rotatePitch(-f).rotateYaw(-f1));
            }
            else
            {
                double d5 = this.getBoundingBox().getYSize() - 1.0D;
                double d6 = this.isCrouching() ? -0.2D : 0.07D;
                return this.func_242282_l(p_241843_1_).add((new Vector3d(d0, d5, d6)).rotateYaw(-f1));
            }
        }
        else
        {
            Vector3d vector3d = this.getLook(p_241843_1_);
            Vector3d vector3d1 = this.getMotion();
            ...
            return this.func_242282_l(p_241843_1_).add((new Vector3d(d0, -0.11D, 0.85D)).func_242988_c(-f2).rotatePitch(-f).rotateYaw(-f1));
        }
    }
